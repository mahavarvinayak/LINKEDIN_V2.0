...
export default function HomePage() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    // Check sessionStorage for abuse prevention
    if (typeof window !== "undefined") {
      const hasAnalyzed = sessionStorage.getItem("has_analyzed");
      if (hasAnalyzed) {
        setValidationError(
          "You've already used your free analysis. Sign up for unlimited access!"
        );
        return;
      }
    }
    ...
    try {
      ...
      // Mark as analyzed in sessionStorage
      if (typeof window !== "undefined") {
        sessionStorage.setItem("has_analyzed", "true");
      }
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-16 sm:pt-24 pb-12">
        ...
        <div className="flex justify-center mb-6">
          <span className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 bg-gray-100 rounded-full px-3 py-1 border border-gray-200">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 text-center leading-tight mb-3">
          Is your LinkedIn post{" "}
          <span className="text-blue-600">scroll-worthy?</span>
        </h1>
        <p className="text-base sm:text-lg text-gray-500 text-center mb-8">
          Paste your post below. Get a free score in 10 seconds.
        </p>
        ...
        <textarea
          ...
          placeholder={`Paste your LinkedIn post here...\n\nExample:\nI just got promoted to Senior Developer.\nHard work pays off.\nKeep going everyone!`}
        ...
        {pageState === "input" && (
          <>
            ...
            <p className="text-xs text-gray-400 text-center mt-3">
              No signup needed. Results in seconds. 100% free.
            </p>
          </>
        )}
      ...
      {pageState === "results" && result && (
        <div
          ref={resultsRef}
          className="max-w-2xl mx-auto px-4 sm:px-6 pb-20 space-y-8"
        >
          {/* Section A — Overall Score */}
          <div className="text-center">
            <p
              className={`text-5xl sm:text-6xl font-bold ${getScoreColor(result.overall_score)}`}
            >
              {result.overall_score.toFixed(1)}{" "}
              <span className="text-2xl text-gray-400 font-normal">/ 10</span>
            </p>
            <p className="text-gray-500 mt-2 text-sm">
              {getOverallText(result.overall_score)}
            </p>
          </div>

          {/* Section B — 4 Score Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <ScoreCard
              name="Hook Strength"
              score={result.scores.hook.score}
              label={result.scores.hook.label}
              explanation={result.scores.hook.explanation}
            />
            <ScoreCard
              name="Readability"
              score={result.scores.readability.score}
              label={result.scores.readability.label}
              explanation={result.scores.readability.explanation}
            />
            <ScoreCard
              name="Engagement"
              score={result.scores.engagement.score}
              label={result.scores.engagement.label}
              explanation={result.scores.engagement.explanation}
            />
            <ScoreCard
              name="Structure"
              score={result.scores.structure.score}
              label={result.scores.structure.label}
              explanation={result.scores.structure.explanation}
            />
          </div>

          {/* Section C — What's Wrong */}
          {result.top_problems && result.top_problems.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-5">
              <h3 className="text-red-700 font-medium mb-3">
                Why this post isn&apos;t performing
              </h3>
          ...
          {result.improved_post && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <span className="inline-flex items-center gap-1 text-xs font-medium bg-green-100 text-green-700 border border-green-200 rounded-full px-2.5 py-0.5">
                  Engagement{" "}
                  {result.scores.engagement.score >= 7
                    ? result.scores.engagement.score
                    : "8"}
                  /10 ✓
                </span>
              </div>
          ...
          <div className="bg-blue-600 rounded-xl p-6 sm:p-8 text-center">
            <h3 className="text-xl font-semibold text-white mb-2">
              Want posts like this, personalized to you?
            </h3>
            <p className="text-blue-100 text-sm mb-5">
              Set up your free profile. Every post becomes tailored to your voice
              and goals.
            </p>
            <Link
              href="/signup"
              className="inline-block bg-white text-blue-600 font-semibold rounded-lg px-6 py-2.5 hover:bg-gray-50 transition-colors"
            >
              Get started free →
            </Link>
